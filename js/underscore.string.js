	...
	var _s = {
		...
		startsWith : function(str, starts) {
			...
			return str.length >= starts.length
					&& str.slice(0, starts.length) === starts;
		},
		...
		dasherize : function(str) {
			return _s.trim(str).replace(/([A-Z])/g, '-$1').replace(/[-_\s]+/g,
					'-').toLowerCase();
		},

		classify : function(str) {
			return _s.capitalize(_s
					.camelize(String(str).replace(/[\W_]/g, ' ')).replace(
							/\s/g, ''));
		},

		humanize : function(str) {
			return _s.capitalize(_s.underscored(str).replace(/_id$/, '')
					.replace(/_/g, ' '));
		},
		...
		trim : function(str, characters) {
			...
			return String(str)
					.replace(
							new RegExp('^' + characters + '+|' + characters
									+ '+$', 'g'), '');
		},
		...
		truncate : function(str, length, truncateStr) {
			...
			length = ~~length;
			return str.length > length ? str.slice(0, length) + truncateStr
					: str;
		},

		/**
		 * _s.prune: a more elegant version of truncate prune extra chars, never
		 * leaving a half-chopped word.
		 * 
		...
		prune : function(str, length, pruneStr) {
			...
			return (template + pruneStr).length > str.length ? str : str.slice(
					0, template.length)
					+ pruneStr;
		},
		...
		numberFormat : function(number, dec, dsep, tsep) {
			...
			var parts = number.split('.'), fnums = parts[0], decimals = parts[1] ? (dsep || '.')
					+ parts[1]
					: '';
		...
		toSentence : function(array, separator, lastSeparator, serial) {
			...
			return a.length ? a.join(separator) + lastSeparator + lastMember
					: lastMember;
		},
		...
		slugify : function(str) {
			...
			var from = "ąàáäâãåæăćęèéëêìíïîłńòóöôõøśșțùúüûñçżź", to = "aaaaaaaaaceeeeeiiiilnoooooosstuuuunczz", regex = new RegExp(
		...
		naturalCmp : function(str1, str2) {
			...
			var cmpRegex = /(\.\d+)|(\d+)|(\D+)/g, tokens1 = String(str1)
					.toLowerCase().match(cmpRegex), tokens2 = String(str2)
					.toLowerCase().match(cmpRegex), count = Math.min(
...
_.mixin(_.str.exports()); // mix in to underscore
